...
const LiveCasionPage = () => {
  ...
  const gamesList = getGames(Categories.LiveDealers, search, provider === "all" ? limit : undefined, provider);
      ...
      <TabLayout>
        <div>
          ...
          <main className="py-5 px-2 bg-[#003e3e] pb-24 md:pb-5 min-h-screen">
            <FilterProivder onSelect={handleProviderChange} category="live_dealers" />
            ...
            {isFilterLoading ? (
              <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-3 lg:gap-4 mt-2">
                <GameLoader length={15} loading={true} category="live_dealers" />
              </div>
            ) : (
              <>
                <div key={provider} className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-3 lg:gap-4 mt-2 provider-list-appear">
                  ...
                  {!gamesList && <GameLoader length={15} loading={true} category="live_dealers" />}
                </div>
